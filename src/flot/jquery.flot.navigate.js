...
(function ($) {
    ...
    function init(plot) {
        ...
        function onMouseMove(plot, e) {
            ...
            //if an event series is selected do not highlight an axis
            /*var eventSelected =  plot.getData()
                .map(s=>s.events.show&&s.events.selected)
                .reduce((e,acc)=>e||acc,false);
            if (eventSelected){
                zoomableAxis = UNSPECIFIED_AXIS;
            }*/
            //...otherwise check if the cursor is close to the time axis
            if (x_val < ZOOM_BOUND && x_val < y1_val && x_val < y2_val) {
                zoomableAxis = X_AXIS;
                if (x_val < RESIZE_BOUND) {
                    resizableAxis = X_AXIS;
                }
            //...otherwise check if the cursor is close the y1 axis
            } else if (y1_val < ZOOM_BOUND && y1_val < x_val) {
                zoomableAxis = Y1_AXIS;
                if (y1_val < RESIZE_BOUND) {
                    resizableAxis = Y1_AXIS;
                }
            //..otherwise check if the cusor is close to the y2 axis
            } else if (y2_val < ZOOM_BOUND && y2_val < x_val) {
                //first check if this plot has a y2 axis
            ...
            //cursor isn't on an event or near an axis
            } else {
                zoomableAxis = UNSPECIFIED_AXIS;
            }
        ...
        plot.zoom = function (args) {
            ...
            //handle zoom on an event stream
             //if an event series is selected do not highlight an axis
             /*var selected_event_series =  plot.getData()
             .filter(s=>s.events.show&&s.events.selected)
             .reduce((acc,s)=>s,null);
            if (selected_event_series!=null){
                if(amount>1)
                    selected_event_series.events.height *= 1.2;
                else
                    selected_event_series.events.height *= 0.8;

                selected_event_series.events.height=Math.max(5,selected_event_series.events.height);
                //plot.getPlaceholder().trigger("eventSettingsChanged", selected_event_series.events);
            }*/
            //handle zoom on the plot axes
            $.each(plot.getAxes(), function (_, axis) {
                ...
                if (zoomableAxis == UNSPECIFIED_AXIS)
                    return; // do not allow multi-axis zooming
        ...
        plot.pan = function (args) {
            ...
            //handle pan on an event stream
            //if an event series is selected do not pan a y-axis axis
             /*var event_panned = false;
             var selected_event_series =  plot.getData()
                .filter(s=>s.events.show&&s.events.selected)
                .reduce((acc,s)=>s,null);
            if (selected_event_series!=null){
                selected_event_series.events.offset -= args.top;
                event_panned=true;
                //plot.getPlaceholder().trigger("eventSettingsChanged", selected_event_series.events);
            ...
            $.each(plot.getAxes(), function (_, axis) {
                ...
                if(axis.n==3 || axis.n==4)
                    return;
                /*//if an event series is selected do not pan a y-axis axis
                if ((axis.direction == "y" && selected_event_series))
                    return;*/
